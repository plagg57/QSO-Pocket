...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const tryAuth = () => {
      ...
      axios.get(`${API}/auth/me`)
        .then(res => { setUser(res.data); setServerWaking(false); setChecking(false); })
        .catch(err => {
          const status = err.response?.status;
          if (status === 401 || status === 403) {
            localStorage.removeItem("qso_token"); setUser(false); setServerWaking(false); setChecking(false);
          } else if (retryCount < maxRetries) {
            retryCount++; setServerWaking(true); setTimeout(tryAuth, delay);
          } else {
            try { const payload = JSON.parse(atob(token.split(".")[1])); setUser({ id: payload.sub, email: payload.email || "", callsign: "...", role: "user" }); }
            catch { setUser(false); }
            setServerWaking(false); setChecking(false);
          }
        });
    };
    tryAuth();
  ...
  const logout = async () => {
    ...
    localStorage.removeItem("qso_token"); localStorage.setItem("qso_logged_out", "true");
    sessionStorage.clear(); delete axios.defaults.headers.common["Authorization"]; setUser(false);
  };
